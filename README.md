...
- cms: 内容管理后台
- task: 常驻脚本任务
> api和cms服务基于web框架gin，task服务基于cli框架cobra。
...
model/                    # 存储模型定义
    ...
    queue/                # 消息队列名称和结构
pkg/                      # 公共方法包
    json/                 # json库标签编译adapter
    logger/               # 日志
    process/              # 进程相关方法
    core/                 # 核心上下文封装，主要为task使用
    web/                  # web服务扩展(api/cms使用)
        errcode/          # 响应错误码定义
        middleware/       # 通用路由中间件方法
    worker/               # 任务调度扩展方法(task使用)
    db/                   # 数据库连接
    gredis/               # redis连接
    gnsq/                 # nsq连接
    storage/              # 云对象存储API
...
### 编译运行
> - 本地运行需将api、cms、task目录下conf.dev.yaml复制为conf.yaml并修改相应配置，可通过命令参数`-c`指定配置文件。
> - 线上部署需在CI发布流水fetch代码后，从配置中心拉取`conf.yaml`到对应的服务目录下，再构建镜像。
> - 本地调式可直接在api、cms、task三个目录下运行`go run .`命令临时编译并运行。
```shell
go run -tags sonic,debug ./api -c ./api/conf.yaml
go run -tags sonic,debug ./cms -c ./cms/conf.yaml
go run -tags sonic,debug ./task cronjob -c ./task/conf.yaml
go run -tags sonic,debug ./task example -c ./task/conf.yaml
```
> `sonic`标签指定gin框架和logger包使用的json库，`debug`标签开启pprof性能分析。

逐个构建docker镜像并启动
```shell
docker build --build-arg srv=api -t api .
docker build --build-arg srv=cms -t cms .
docker build --build-arg srv=task -t task .
```
```shell
docker run -d -e TZ=Asia/Shanghai -p 8000:8000 --name api api
docker run -d -e TZ=Asia/Shanghai -p 6000:6000 --name cms cms
docker run -d -e TZ=Asia/Shanghai --name cronjob task cronjob
docker run -d -e TZ=Asia/Shanghai --name example task example
```

### 日志设计
- 基于标准库原生封装，支持控制台标准输出、格式化输出、文件输出。通过配置app.logger.output指定。
- 容器部署可根据日志收集策略指定为`std`标准输出(采集方便)或`file`文件输出(性能最优)，本地调试可指定为`fmt`格式化输出(显示友好)。
...
- output 结果、响应体……
- elapsed （耗时环节）经过的毫秒数
> v0,v1,v2,v3通过context传递，串联起完整链路
#### 使用示例
```
//单次打印
