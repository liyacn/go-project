...
### 编译运行
> - 本地运行需将api、cms、task目录下conf.dev.yaml复制为conf.yaml并修改相应配置，也可以通过命令参数`-c`指定配置文件。
...
(cd task && go run -tags sonic,debug . cronjob)
(cd task && go run -tags sonic,debug . example)
```
