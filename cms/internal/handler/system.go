...
func (h *Handler) SystemActionSync(g *gin.Engine) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		oldActionSet := make(map[string]struct{})
		oldGroupSet := make(map[string]struct{})
		for _, v := range aks {
			if strings.HasPrefix(v, "/") {
				oldActionSet[v] = struct{}{}
			} else {
				oldGroupSet[v] = struct{}{}
			}
		}
		newGroupSet := make(map[string]struct{})
		var newGroupKeys []string
		routes := g.Routes()
		...
		for _, v := range routes {
			...
			if _, ok := newGroupSet[paths[0]]; !ok {
				newGroupSet[paths[0]] = struct{}{}
				newGroupKeys = append(newGroupKeys, paths[0])
			}
			if _, ok := oldActionSet[v.Path]; ok { // 存在则跳过
				delete(oldActionSet, v.Path)
			} else { // 不存在则插入
		...
		sortNum = 1
		for _, key := range newGroupKeys {
			if _, ok := oldGroupSet[key]; ok { // 存在则跳过
				delete(oldGroupSet, key)
			} else { // 不存在则插入
				actionCreate = append(actionCreate, &entity.SystemAction{
					KeyName: key,
					Sort:    sortNum,
		...
		actionDelete := append(types.Keys(oldActionSet), types.Keys(oldGroupSet)...) // 多余的删除
