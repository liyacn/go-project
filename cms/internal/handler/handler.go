...
func NewEngine(cfg *Config, srv *service.Service) *gin.Engine {
...
func (h *Handler) RBAC(c *gin.Context) {
	auth := getAuth(c)
	if !auth.IsSuper {
		actions, err := h.service.AdminRoleActions(c, auth.RoleID)
		if err != nil {
			logger.FromContext(c).Error("service.AdminRoleActions error", auth.RoleID, err)
			c.AbortWithStatusJSON(errcode.FromError(err))
			return
		}
		if !slices.Contains(actions, c.Request.URL.Path) {
