...
func (s *Service) AdminTokenSet(ctx context.Context, data *cache.AdminToken) (string, error) {
	...
	_, err = s.redis.TxPipelined(ctx, func(pipe redis.Pipeliner) error {
		if oldToken != "" {
			pipe.Unlink(ctx, cache.AdminTokenKey(oldToken))
		}
		pipe.Set(ctx, cache.AdminTokenKey(newToken), b, time.Hour)
		pipe.Set(ctx, ssoKey, newToken, time.Hour)
		return nil
	})
	return newToken, err
}
...
func (s *Service) AdminTokenGet(ctx context.Context, token string) (*cache.AdminToken, error) {
	...
	if _, e := s.redis.TxPipelined(ctx, func(pipe redis.Pipeliner) error {
		pipe.Expire(ctx, key, time.Hour)
		pipe.Expire(ctx, cache.AdminSsoKey(result.ID), time.Hour)
		return nil
	}); e != nil {
		logger.FromContext(ctx).Error("redis.TxPipelined.Exec error", e)
	}
	return &result, err
}
