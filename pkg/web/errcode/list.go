...
var ( // 400xx BadRequest
	ParamsInvalid       = ErrCode{40000, "参数错误"}  // 参数基本校验失败
	AssociatedIdInvalid = ErrCode{40001, "无效的参数"} // 提交数据关联的ID无效
)
...
var ( // 500xx InternalServerError
	...
	CommonServerError   = ErrCode{50001, "系统繁忙"} // 服务端通用错误
	RedisServerError    = ErrCode{50002, "系统繁忙"}
	NsqServerError      = ErrCode{50003, "系统繁忙"}
)
