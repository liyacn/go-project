...
func (h *Handler) AdminRoleSave(c *gin.Context) {
	...
	if r.ID != 0 {
		...
		if r.Name == role.Name &&
			types.CheckEqualSet(r.Actions, role.Actions) &&
			types.CheckEqualSet(r.Menus, role.Menus) {
			c.JSON(OK, nil)
			return
		}
	}
	...
	if err != nil {
		logger.FromContext(c).Error("service.AdminRoleSave error", nil, err)
		c.JSON(errcode.FromError(err))
		return
	}
	c.JSON(OK, nil)
