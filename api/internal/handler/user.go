...
func (h *Handler) WechatLogin(c *gin.Context) {
	...
	if err != nil {
		logger.FromContext(c).Error("service.UserSave error", err)
		c.JSON(errcode.FromError(err))
		return
	}
...
func (h *Handler) WechatPhone(c *gin.Context) {
	...
	err = h.service.UserUpdate(c, &entity.User{
		ID:          user.ID,
		PhoneNumber: h.aes.Encrypt([]byte(resp.PhoneInfo.PhoneNumber)),
	})
	if err != nil {
...
func (h *Handler) UserProfile(c *gin.Context) {
	...
	if strings.HasPrefix(r.AvatarURL, h.cdn) {
		r.AvatarURL = ""
	}
	user := getAuth(c)
	err := h.service.UserUpdate(c, &entity.User{
	...
	if r.AvatarURL != "" {
		if err = h.service.AvatarToCdnAsync(&queue.AvatarToCdn{
			...
			logger.FromContext(c).Error("service.AvatarToCdnAsync error", err)
		}
	}
	c.JSON(OK, nil)
}
...
func (h *Handler) UserInfo(c *gin.Context) {
	...
	if info.PhoneNumber != "" {
		resp.PhoneNumber = string(h.aes.Decrypt(info.PhoneNumber))
	}
	c.JSON(OK, resp)
}
